...
(
    ...
    input                   reset,
    input                   stall,
    output reg              exception, //signals illegal instruction or misaligned fetch
    ...
    input                   stall_read_i,
    input  [31:0]           inst_fetch_pc,//storing address of instruction (address is also 32 bits wide) 
    input  [31:0]           instruction_i, //validated instruction comin frm the IF stage which gets accepted by the ID stage

    // -----------------------------    // WB-stage signals (passed in)    // -----------------------------
    input                   wb_stall, //write back (used for hazard detection)
    input                   wb_alu_to_reg,
    input                   wb_mem_to_reg,
...
reg  [31:0] immediate;
reg         illegal_inst; //initialised inside the combinational always block nd then after checking opcode with instruction_i, we are deciding whether illegal or not(default block inside case)
...
//instruction_o --> output of IF_ID (it is the instruction that will be seen by the ID stage)
assign instruction_o = stall_read_i ? NOP : inst_mem_read_data ; //we declared this reg earlier as the validated version of the instruction
...
always @(posedge clk or negedge reset) begin
    if (!reset)
        exception <= 1'b0; //on reset, clear exception
    else if (illegal_inst || inst_mem_offset != 2'b00) //since PC is a multiple of 4, the last 2 bits must be 00 (binary ver)
        exception <= 1'b1;
    else
        exception <= 1'b0;
end
...
always @(*) begin //combinational logic
    //in memory, the opcode is of 12 bits, bt when generating immediate we need to have 32 bits, so we need to perform sign extension
    //immediate is encoded using fewer bits (12 in our case) bt generated and used as 32 bits
	immediate	= 32'h0;
	illegal_inst = 1'b0;
	...
	case (instruction_i[`OPCODE])
    	...
    	// JALR:
    	// Lower 12 bits  = instruction_i[31:20] (the actual opcode)
    	// Upper 20 bits  = Sign-extend
    	JALR  : immediate = {{20{instruction_i[31]}}, instruction_i[31:20]}; //we are writing the 31st bit 20 times => sign extending at the remaining places

        
    	// BRANCH: basically we are branching frm our straight route to go to some other location bcoz our if/any other loop has prompted us to go there for our next process
    	//to store that offset (PC = PC + offset) we need immediates (bt since there's no consecutive space to store immediates, they are scattered everywhere)
    	// immediate[12]   = instruction_i[31]   (sign bit -> direction of jump)
    	// immediate[11]   = instruction_i[7]
    	// immediate[10:5] = instruction_i[30:25]
    	// immediate[4:1]  = instruction_i[11:8]
    	// immediate[0]	= 1'b0
    	// immediate[31:13]= Sign-extend //only the first 12 bits are imp, remaining have to be sign extended)
...
//instantianting the module id ex register
id_ex_reg u_id_ex (
	.clk        	(clk),
	.reset_n    	(reset),
	...
	.arithsubtype_i (
    	...
    	!(instruction_i[`OPCODE] == ARITHI && instruction_i[`FUNC3] == ADD)
	),
	.pc_i       	(inst_fetch_pc),
...
always @(posedge clk or negedge reset_n) begin
    ...
    else if (!stall_n) begin // !stall_n means that we dont wanna stall, run it normally => input signals copied to output signals (the input signals are coming frm the opcode)
    //if stall is 1, this block doesnt get executed, so the outputs hold their prev values
